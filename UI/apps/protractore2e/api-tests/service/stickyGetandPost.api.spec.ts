...
let sampleBody;
let validBody;
describe('Sample Sticky POST', async () => {
    ...
    it('Validate the post status code', async () => {
        response = await StickyNoteAPI.postSticky('98', validBody);
        expect(response.status).toBe(200);
    });
});

describe('Sample Sticky GET', async () => {
    it('Validate the get status code', async () => {
        response = await StickyNoteAPI.getSticky('98');
        expect(response.status).toBe(200);
    });
